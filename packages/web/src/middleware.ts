...
export function middleware(_request: NextRequest) {
  ...
  // Umami analytics origin (optional)
  let umamiOrigin = '';
  ...
  if (process.env.NODE_ENV === 'production') {
    response.headers.set(
      ...
      `default-src 'self'; script-src 'self' 'unsafe-inline' https://accounts.google.com https://apis.google.com${umamiOrigin ? ` ${umamiOrigin}` : ''}; style-src 'self' 'unsafe-inline' https://accounts.google.com; img-src 'self' data: https:; font-src 'self' data: https://fonts.gstatic.com; frame-src https://accounts.google.com; connect-src 'self' ${apiOrigin} https://accounts.google.com${umamiOrigin ? ` ${umamiOrigin}` : ''}`
    );
